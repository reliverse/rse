...
// https://github.com/AmanVarshney01/create-better-t-stack/blob/main/apps/cli/src/helpers/database-providers/neon-setup.ts
...
async function createNeonProject(
  ...
  try {
    ...
    if (
      ...
      const connectionUri =
        response.connection_uris.at(0)?.connection_uri ?? "";
      const params =
        response.connection_uris.at(0)?.connection_parameters ?? "";
