...
// https://github.com/AmanVarshney01/create-better-t-stack/blob/main/apps/cli/src/helpers/core/api-setup.ts
...
export async function setupApi(config: ProjectConfig) {
  ...
  if (isConvex) {
    ...
    if (nativeDirExists) {
      await addPackageDependency({
        dependencies: convexDeps.native!
          .dependencies as AvailableDependencies[],
        projectDir: nativeDir,
      });
    }
