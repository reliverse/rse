...
export const rseSchema = Type.Object({
  // rseg schema
  $schema: Type.Union([
    Type.Literal(RSE_SCHEMA_URL),
    Type.Literal(RSE_SCHEMA_DEV),
  ]),
...
/**
 * A helper to define a rseguration in the rse.tsrse
 */
export function defineConfig<T extends RseConfig>(config: T): T {
  return config;
}
